        ...
        <>
            <div className='bg-one py-24 dark:bg-five dark:text-six'>
                ...
                <div className="grid w-full place-items-center space-y-4 px-2 py-10 md:grid-cols-2 md:gap-6 md:space-y-0 lg:grid-cols-3">
                    {data
                        .filter((car) =>
                            car.MakeOfCar.toLowerCase().includes(searchTerm.toLowerCase()) || car.Model.toLowerCase().includes(searchTerm.toLowerCase())
                        )
                        .map((car) => {
                            return (
                                <div
                                    key={car._id}
                                    className="rounded-t-lg border w-4/5 hover:scale-105 h-full shadow-lg shadow-five dark:shadow-one"
                                >
                                    <img
                                        src={car.FrontView}
                                        alt="Car"
                ...
                {/* Add this conditional rendering for no results */}
                {searchTerm &&
                    data.filter(car =>
                        car.MakeOfCar.toLowerCase().includes(searchTerm.toLowerCase()) ||
